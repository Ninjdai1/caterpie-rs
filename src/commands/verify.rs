...
use serenity::all::{ButtonStyle, CommandInteraction, Context, CreateActionRow, CreateButton, CreateCommandOption, CreateEmbed, EditInteractionResponse, EditMessage, ResolvedOption, ResolvedValue};
...
pub async fn run(h: &Handler, ctx: &Context, command: &CommandInteraction) -> Result<(), serenity::Error> {
    ...
    let user_id = match &command.data.options().first() {
        Some(ResolvedOption {value: ResolvedValue::User(user, _), ..}) => Some(user.id.get()),
        _ => None
    };
    ...
    loop {
        ...
        let builder = EditInteractionResponse::new().embed(data.0).components(match data.1 {
            Some(r) => vec![r],
            None => vec![]
        });
...
pub fn register() -> CreateCommand {
    CreateCommand::new("verify")
        ...
        .add_option(CreateCommandOption::new(serenity::all::CommandOptionType::User, "user", "A specific user to verify submissions for"))
}

pub async fn create_verification_message(h: &Handler, user: Option<u64>) -> (CreateEmbed, Option<CreateActionRow>, u32) {
