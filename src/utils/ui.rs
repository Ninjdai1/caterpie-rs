...
use crate::{entities::{actions::ActionType, prelude::*}, Handler, CONFIG};
...
pub async fn update_permanent_leaderboard(db_conn: &DatabaseConnection, ctx: &Context) {
    if let Err(err) = EditMessage::new().embed(generate_leaderboard_embed(db_conn, None, None).await).execute(&ctx.http, (CONFIG.permanent_leaderboard.0, CONFIG.permanent_leaderboard.1, None)).await {
        print!("e");
        log::error!("Error while editing permanent leaderboard: {err:?}");
    }
}

pub async fn generate_leaderboard_embed(db_conn: &DatabaseConnection, action_type: Option<ActionType>, id: Option<u64>) -> CreateEmbed {
...
fn generate_leaderboard_string(score_vec: &mut Vec<(&String, &Score)>, action_type: Option<ActionType>, id: Option<u64>, max: usize) -> String {
    ...
    let mut display_user = match id {None=>false, Some(_)=>true};
    ...
    if display_user {
        if let Some(pos) = score_vec.iter().position(|u| *u.0 == id.unwrap().to_string()) {
            let u = score_vec[pos];
            res_str.push_str(&format!("\n**{} <@{}>: {} {}** (You)",
                format!("#{}", pos+1),
                u.0,
